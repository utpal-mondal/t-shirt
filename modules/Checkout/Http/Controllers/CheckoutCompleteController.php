...
class CheckoutCompleteController
{
    ...
    public function store(int $orderId, OrderService $orderService)
    {
        ...
        if (request()->query('paymentMethod') === 'bkash') {
            $redirectToCancel = redirect()
                ->route('checkout.payment_canceled.store', [
                    'orderId' => $orderId,
                    'paymentMethod' => 'bkash',
                ]);
            ...
            try {
                ...
                $bkashPayment = new BkashPayment($bkashConfig);
                $paymentDetails = $bkashPayment->query($paymentId);

                if (!$paymentDetails || strtolower($paymentDetails->transactionStatus) !== 'completed') {
                    return $redirectToCancel;
                }
